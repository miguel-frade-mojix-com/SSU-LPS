...
public class AgendaDAO extends DataBaseConnector {
    ...
    public static LinkedList<Agenda> getAgendados(Date fechaConsulta, String nombreMedico){
        ...
        try{
           ...
           while(rs.next()){
               logger.log(Level.INFO,"Agenda Result" + rs.getString(1) + " " + rs.getString(2) +  " "  + rs.getDate(3) + " " + rs.getInt(4) + " turno:" + rs.getInt(6)   );
               ...
               while(rs2.next()){
                   logger.log(Level.INFO,"Beneficiario agendado: " +rs2.getString("Primer_Nombre") + " " + rs2.getString("Primer_Apellido")    );
            ...
            logger.log(Level.WARNING,"Catch from getAgendados" + ex.getMessage());
...
//        String apellidoMaterno = campos[2];
        ...
        try{
           ...
           rs =pst.executeQuery();
            ...
            while(rs.next()){
                logger.log(Level.INFO,"Result : "+rs.getString(1) );//+ " " + rs.getString(2) + " " + rs.getNString(3) );
                result = rs.getString("ID_Medico");
            }
            
        }catch(SQLException ex ){
            logger.log(Level.WARNING,"Controllers.AgendaDAO.getIDMedico()" + ex.getMessage()); 
        }finally{
    ...
    public static void  agendarPaciente(Beneficiario ben,Date fecha,int turno ){
        ...
        if(agendado == true){
            ...
            try{
                ...
                logger.log(Level.WARNING,"Agendar pacientes has failed:  "+ ex.getMessage());
            }finally{
                   try{
                   if(con!=null)
                       con.close();
    ...
    public static HashSet<Integer> verHorarioDisponible(Date fecha){
        ...
        try{
            ...
            logger.log(Level.WARNING,"Error al conectar con base de datos para buscar fechas disponibles. Error:  "+ ex.getMessage());
           ...
           try{
               ...
               logger.log(Level.WARNING,"Controllers.AgendaDAO.verHorarioDisponible()");
           }    
        }    
        return horarios;
    }
